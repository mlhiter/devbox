...
export async function checkNewHostInHostkeys(host: string): Promise<boolean> {
  const fileContent = await fs.promises.readFile(KNOW_HOST_FILE, {
    encoding: 'utf8',
  })
  ...
  for (let line of lines) {
    ...
    const [salt_, hostHash_] = hostEncrypted_
      .substring(HASH_MAGIC.length)
      .split(HASH_DELIM)
    const hostHash = crypto
      .createHmac('sha1', Buffer.from(salt_, 'base64'))
      .update(host)
      .digest()
...
export async function addHostToHostFile(
  ...
  const entry = `${HASH_MAGIC}${salt.toString(
    'base64'
  )}${HASH_DELIM}${hostHash.toString('base64')} ${type} ${hostKey.toString(
    'base64'
  )}\n`
